...
def run_command(np_int):
	...
	os.system(command)
	if np_int == 1:
		command = "xmake NUM_PORTS=1"
	elif np_int == 2:
		command = "xmake NUM_PORTS=2"
	elif np_int == 7:
		command = "xmake NUM_PORTS=7"
	elif np_int == 8:
		command = "xmake NUM_PORTS=8"
	elif np_int == 15:
		command = "xmake NUM_PORTS=15"
	elif np_int == 16:
		command = "xmake NUM_PORTS=16"
	import os
	os.system(command)
...
while((resolution != '') & (run_while == 1) & (ind_test == 0) & (inp_data_val == 1)):
	...
	print (ts_int + 1)
	hdr.write(ts)
	hdr.write("# define NUM_PORTS ")	
	hdr.write(np)
	print (np_int +1)
	hdr.write("\n")
	hdr.write("unsigned int value[NUM_PORTS] = {0}; ")
	hdr.write("\n")
	hdr.write("unsigned int mod_type = ")
